...
class DailyPaymentStatsViewModel(private val repository: Repository) : ViewModel() {

    fun getDailyPaymentStats(): LiveData<NetworkStates<Response<BaseResponse<DailyPaymentStatsData>>>> =
        liveData(Dispatchers.IO) {
            emit(NetworkStates.loading(null))
            try {
                val response = repository.getDailyPaymentStats() // ✅ no token
